...
import { ChevronLeft, ShieldCheck, XCircle, Package, Hash, CheckCircle2, AlertCircle, Loader2 } from "lucide-react";
  ...
  const loadData = async () => {
    try {
      ...
      if (res && res.success && res.data) {
        // According to your JSON: res.data contains total_approved, total_rejected, and products array
      ...
      setError("Failed to load certification summary. Please try again.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="bg-slate-50 min-h-screen w-full p-4 sm:p-8 font-sans">
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-500 text-sm hidden sm:block">Manage and view your Kalki-Quality verified inventory</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-white p-5 rounded-2xl border border-rose-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Total Rejected</p>
                ...
                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <Package size={16} className="text-slate-300 group-hover:text-blue-500 transition-colors" />
                      <span className="font-bold text-slate-700">{item.name}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    {/* Logic: 1 = Approved, 2 = Rejected */}
                    {item.kalki_certified === 1 ? (
                      ...
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase bg-rose-100 text-rose-700">
                        <XCircle size={12} /> Rejected
                      </span>
                    )}
                  </td>
                  <td className="px-6 py-4 text-slate-500 text-sm italic">
                    {item.reason ? `"${item.reason}"` : "No remarks available"}
                  </td>
                </tr>
              )) : (
                <tr>
        ...
        <div className="md:hidden divide-y divide-slate-100">
          {products.map((item, index) => (
            <div key={item.id} className="p-5 bg-white relative overflow-hidden">
              ...
              <div className={`absolute left-0 top-0 bottom-0 w-1 ${item.kalki_certified === 1 ? "bg-emerald-500" : "bg-rose-500"}`} />
      ...
      <div className="max-w-7xl mx-auto mt-6 text-center">
        <p className="text-slate-400 text-xs">
          Showing {products.length} certification audit results.
        </p>
      </div>
    </div>
  );
