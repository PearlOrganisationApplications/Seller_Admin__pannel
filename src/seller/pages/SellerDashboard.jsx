...
import { 
  Plus, Wallet, ShoppingBag, Package, 
    ...
    <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-8">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard title="Monthly Earnings" value={formatINR(dashboardData?.earnings?.current_month)} icon={<Wallet className="text-blue-600" />} color="bg-blue-50" />
          <StatCard title="Withdraw Balance" value={formatINR(dashboardData?.earnings?.withdraw_balance)} icon={<ShoppingBag className="text-purple-600" />} color="bg-purple-50" />
          <StatCard title="Total Orders" value={dashboardData?.orders?.total} icon={<Package className="text-orange-600" />} color="bg-orange-50" />
          <StatCard title="Pending" value={dashboardData?.orders?.pending} icon={<Clock className="text-amber-600" />} color="bg-amber-50" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <AreaChart data={chartData}>
                  <defs>
                    <linearGradient id="colorOrders" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="#1E4CF6" stopOpacity={0.15}/>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center p-3 bg-gray-50 rounded-xl">
                  <span className="text-gray-600 text-sm font-medium">Total Listed</span>
                  <span className="font-bold text-lg">{dashboardData?.products?.total}</span>
                </div>
                <div className="flex justify-between items-center p-3 bg-green-50 rounded-xl">
                  <span className="text-green-700 text-sm font-medium">Active</span>
                  <span className="font-bold text-green-700 text-lg">{dashboardData?.products?.active}</span>
                </div>
                <div className="flex justify-between items-center p-3 bg-red-50 rounded-xl">
                  <span className="text-red-700 text-sm font-medium">Inactive</span>
                  <span className="font-bold text-red-700 text-lg">{dashboardData?.products?.inactive}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-[#138A63] rounded-2xl p-6 shadow-lg text-white relative overflow-hidden">
               ...
               <p className="text-sm opacity-80 font-medium">Order Fulfillment</p>
               <h4 className="text-3xl font-bold mt-1">{dashboardData?.orders?.completed}</h4>
               <p className="text-xs mt-4 bg-white/20 inline-block px-2 py-1 rounded">Successfully Delivered</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-6 border-b border-gray-50 flex items-center justify-between">
            <h3 className="font-bold text-lg text-gray-800">Recent Return Orders</h3>
                ...
                {dashboardData?.return_orders?.data?.length > 0 ? (
                  dashboardData.return_orders.data.map((order, i) => (
                    <tr key={i} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 font-semibold text-gray-700">{formatINR(order.amount)}</td>
