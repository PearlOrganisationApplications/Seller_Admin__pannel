...
import { FaArrowLeft, FaEdit, FaGavel, FaShieldAlt, FaSyncAlt, FaTimes, FaSave, FaCheckCircle } from "react-icons/fa";
    ...
    <div className="min-h-screen bg-[#F1F2F4] p-4 md:p-8 font-sans relative">
      ...
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate(-1)} className="bg-[#0047AB] text-white p-2.5 rounded-lg shadow-md hover:bg-blue-800 transition-all">
            <FaArrowLeft size={18} />
          </button>
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Legal & Policies</h1>
            <p className="text-sm text-gray-500">View and update platform legal documentation</p>
          </div>
        </div>
        <button onClick={fetchContent} className="flex items-center gap-2 bg-white text-[#0047AB] border border-[#0047AB] px-6 py-2 rounded-lg font-bold hover:bg-blue-50 transition-all shadow-sm">
          ...
          policies.map((item) => (
            <div key={item.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-50 flex justify-between items-center bg-gray-50/30">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-blue-50 rounded-lg text-[#0047AB]">
                    {item.title.toLowerCase().includes('terms') ? <FaGavel size={18}/> : <FaShieldAlt size={18}/>}
                  </div>
                  <h2 className="text-lg font-bold text-gray-800 capitalize">{item.title}</h2>
                </div>
                <button 
              ...
              <div className="p-6 md:p-8">
                <p className="text-gray-600 leading-relaxed text-lg whitespace-pre-wrap">{item.description}</p>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleUpdate} className="p-6 md:p-8 space-y-6">
              <div>
                <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Policy Title</label>
                <input 
                  type="text"
                  required
              ...
              <div>
                <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Description Content</label>
                <textarea 
                  required
                  rows="8"
